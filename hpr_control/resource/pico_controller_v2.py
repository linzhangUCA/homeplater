...
class Motor:
    ...
    def __init__(self, dir_pin, pwm_pin, enca_pin, encb_pin, ab_order=1, frequency=1000):
        ...
        self._velocity_monitor.init(mode=Timer.PERIODIC, period=self.monitor_period, callback=self._velmon_cb)
        self._velocity_controller = Timer()
        self._velocity_controller.init(mode=Timer.PERIODIC, period=self.controller_period, callback=self._velcon_cb)
...
class Robot:
    ...
    def __init__(self, left_motor_pins, right_motor_pins, frequency=1000):
        ...
        self.monitor_period = 10  # millisecond
        # self.rx_period = 20  # millisecond
        # variables
        self.linear_velocity = 0.  # actual velocity
        self.angular_velocity = 0.
        self.target_lin = 0.  # target velocity
        self.target_ang = 0.
        # set a timer to read and send target and actual velocity via usb serial port
        self._velocity_monitor = Timer()
        self._velocity_monitor.init(mode=Timer.PERIODIC, period=self.monitor_period, callback=self._velmon_cb)
        # self._velocity_receiver = Timer()
        # self._velocity_receiver.init(mode=Timer.PERIODIC, period=self.rx_period, callback=self._velrx_cb)

    def _velmon_cb(self, timer):
        """
        Compute robot's actual linear and angular velocity
        """
    ...
    #         self.set_velocity(target_lin, target_ang)
    #         self.led.value(1)
    #     else:
    #         self.led.value(0)
